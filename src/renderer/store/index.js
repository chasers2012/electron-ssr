...
import {
  merge, clone, request, isSubscribeContentValid, getUpdatedKeys, isConfigEqual, somePromise,
} from '../../shared/utils';
import Config from '../../shared/ssr';
import { syncConfig } from '../ipc';
import {
  STORE_KEY_FEATURE, STORE_KEY_SSR_METHODS, STORE_KEY_SSR_PROTOCOLS, STORE_KEY_SSR_OBFSES,
} from '../constants';
...
export default new Vuex.Store({
  ...
  actions: {
    ...
    updateSubscribes({ state, dispatch }, updateSubscribes) {
      ...
      // 累计更新节点数
      let updatedCount = 0;
      return Promise.all(updateSubscribes.map((subscribe) => somePromise([request(subscribe.URL, true), fetch(subscribe.URL).then((res) => res.text())]).then((res) => {
